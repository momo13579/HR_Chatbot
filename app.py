...
5. 【重要排版規定】：請「絕對不要」使用 Markdown 的大標題語法 (例如單一 # 或雙 ##)，這會讓網頁字體過大難以閱讀。請改用粗體字 (**重點文字**) 或是列點 (1. 2. 3.) 的方式來突顯重點。
6. 【重要排版規定】：列出條文或步驟參考時，請務必按照「先後順序（例如由第1條到第2條）」或「邏輯順序（由上到下）」排列，次序絕對不可以顛倒。
